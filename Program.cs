...
for (int i = 0; i < qnt; i++)
{
    Console.WriteLine($"Dados do Livro{i}");
    Console.WriteLine("Nome: "); string livronome = Console.ReadLine() ;
    Console.WriteLine("Preço: "); double preco = double.Parse(Console.ReadLine());
    Console.WriteLine("Peso: "); double peso = double.Parse(Console.ReadLine());
    Livro livro = new(livronome, preco, peso);
    cliente.AdicionarLvros(livro);
}
